...
public class DateFilter extends TokenFilter {
	...
	public boolean increment() throws TokenizerException {
		...
		while (tStreamOld.hasNext()) {
			...
			prev = (tStreamOld.hasTokenbyIndex(tStreamOld.getIndex() - 2)) ? tStreamOld
					...
					: "";
			System.out.println("before: "+cur + " " + next1 + " " + next2 + " " + prev);
			if (list.contains(cur.toLowerCase())) {
				...
				//System.out.println("Before date:" + month + " " + year + " " + date);
				System.out.println(convertDate(year, month, date));
				if(flag){
					if(comma)
						tStreamOld.replaceTokens(1,convertDate(year, month, date)+",");
					else
						tStreamOld.replaceTokens(1,convertDate(year, month, date));
				}
					...
					if(comma)
						tStreamOld.replaceTokens(2,convertDate(year, month, date)+",");
					else
						tStreamOld.replaceTokens(2,convertDate(year, month, date));
				}
				...
				if (years.length > 1) {
					...
					years[1] = years[0].substring(0, 2).concat(
							years[1].replace(",", ""));
				...
				tStreamOld.replaceTokens(5,convertTime(part[0], part[1], sec, next1));
				//System.out.println(convertTime(part[0], part[1], sec, next1));
			} else if (cur.matches("^(\\d{1,2}:\\d{2})(:\\d{2})?([APap][mM])?$")) {
				...
				year = prev;
				if(cur.equals("BC"))
					tStreamOld.replaceTokens(3,convertBC_AD(year, month, date, "BC"));
					//System.out.println(convertBC_AD(year, month, date, "BC"));
				else
					tStreamOld.replaceTokens(3,convertBC_AD(year, month, date, "AD"));
					//System.out.println(convertBC_AD(year, month, date, "AD"));
				System.out.println("after: "+cur + " " + next1 + " " + next2 + " " + prev);
			}
			else if(cur.matches("^\\d{1,4}((BC)|(AD))$")){
				year = cur.substring(0,cur.length()-2);
				System.out.println(convertBC_AD(year, month, date, cur.substring(cur.length()-2)));
			}
	...
	public String convertBC_AD(String y, String m, String d,String type) {
		String reportDate = "";
		try {
			Date newDate = new SimpleDateFormat("yyyy,MMMM,dd").parse(y + ","+ m + "," + d);
			reportDate = new SimpleDateFormat("yyyyMMdd").format(newDate);
			if(type.equals("BC")){
	...
	/*public String prependZero(String str){
	       String s= "0000"+str; // max four zeros
	       return s.substring(s.length()-4); // keep the rightmost 4 chars
	}
*/
